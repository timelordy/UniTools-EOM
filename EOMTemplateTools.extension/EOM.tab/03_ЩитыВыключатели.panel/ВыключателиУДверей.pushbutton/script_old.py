...
def _choose_rooms_for_switch(fr, tr, wet_rx, corridor_rx, has_natural_light_fn):
    ...
    Rules:
    - Wet rooms (bathroom/WC): switch placed OUTSIDE in corridor/hallway
    - All other rooms: switch placed INSIDE the room
    """
    ...
    # 2) For non-wet rooms: switch goes INSIDE the controlled room.
    # Controlled room = the room that is NOT a corridor.
    controlled = None
    other = None
    ...
    if fr_corr and (not tr_corr) and tr is not None:
        ...
        # Both are non-corridor rooms - pick the larger one as controlled
    ...
    # 3) For non-wet rooms: ALWAYS place switch INSIDE the controlled room
...
def _make_linked_room_key(link_inst, room):
    """Return stable key for a linked room within a specific link instance."""
    if link_inst is None or room is None:
        return None
    try:
        li_uid = str(getattr(link_inst, 'UniqueId', '') or '')
    except Exception:
        li_uid = ''
    try:
        r_uid = str(getattr(room, 'UniqueId', '') or '')
    except Exception:
        r_uid = ''

    if li_uid and r_uid:
        return u'{0}::room::{1}'.format(li_uid, r_uid)
    ...
    try:
        ...
        li_id = None
    try:
        r_id = int(room.Id.IntegerValue)
    except Exception:
        r_id = None
    if li_id is not None and r_id is not None:
        return u'{0}::room::{1}'.format(li_id, r_id)
    return None


def _comment_with_room_key(comment_prefix, room_key):
    """Create comment with room key for tracking placed switches per room."""
    if not comment_prefix:
        return comment_prefix
    if not room_key:
        return comment_prefix
    return u'{0}|room={1}'.format(comment_prefix, room_key)


def _collect_existing_switch_room_keys(host_doc, comment_prefix):
    """Return a set of room keys already present in Comments."""
    keys = set()
...
def _collect_doors_for_room(room, doors, link_doc):
    """Find all doors that have this room as FromRoom or ToRoom.

    Returns list of (door, other_room, phase) tuples where other_room is the room on the other side.
    """
    if room is None or not doors:
        return []

    room_id = _room_id_int(room)
    if room_id is None:
        return []
    ...
    for d in doors:
        ...
        fr_id = _room_id_int(fr)
        tr_id = _room_id_int(tr)

        if fr_id == room_id:
            result.append((d, tr, ph))
        elif tr_id == room_id:
            result.append((d, fr, ph))

    return result


def _pick_main_door_for_room(room, room_doors, corridor_rx, wet_rx):
    """Pick the main/entry door for a room.

    Priority:
    1. Door leading to corridor/hallway (main entry)
    2. Door NOT leading to wet room
    3. Widest door
    4. First available door

    room_doors is list of (door, other_room, phase) tuples.
    Returns (best_door, other_room, phase) or (None, None, None).
    """
    if not room_doors:
        return None, None, None
    ...
    scored = []
    for d, other, ph in room_doors:
        score = 0
        other_name = _room_name(other)

        if _match_any(corridor_rx, other_name):
            score += 100

        if other is not None and not _match_any(wet_rx, other_name):
            score += 10

        try:
            width = _get_door_width_ft(d)
            if width is not None:
                score += float(width)
        except Exception:
            pass

        scored.append((score, d, other, ph))
...
def _desired_facing_host_from_link_wall_face(door_link, link_inst, pt_link, total_transform):
    ...
    if wall_link is not None and isinstance(wall_link, DB.Wall):
        try:
            # For facing direction, we want the face that faces TOWARD the room point,
            # so we can compute the vector from wall surface to room.
            _, proj_pt_link = _get_linked_wall_face_ref_and_point(wall_link, link_inst, pt_link, prefer_facing_away=False)
        except Exception:
            proj_pt_link = None
...
def _get_linked_wall_face_ref_and_point(link_wall, link_inst, point_link, prefer_facing_away=True):
    """Return (link_face_ref, projected_point_link) or (None, None).
    
    Args:
        link_wall: The wall element in the linked document.
        link_inst: The RevitLinkInstance.
        point_link: The target point in link coordinates (typically inside the room).
        prefer_facing_away: If True, prefer the face whose normal points AWAY from point_link.
                           This is needed for face-hosted families where we want the device
                           to be embedded IN the wall with its front facing the room.
    """
    ...
    try:
        ...
        for r in refs:
            ...
            # Get face normal to determine which way it's facing
            face_normal = None
            try:
                # For planar faces, compute normal at the projected point
                face_normal = face.ComputeNormal(ir.UVPoint)
            except Exception:
                pass
            if face_normal is None:
                try:
                    # Fallback: use face normal at origin
                    face_normal = face.FaceNormal
                except Exception:
                    face_normal = None
            
            # Compute direction from projected point to target point (room point)
            dir_to_room = None
            try:
                dir_to_room = DB.XYZ(
                    float(point_link.X) - float(pt.X),
                    float(point_link.Y) - float(pt.Y),
                    float(point_link.Z) - float(pt.Z)
                )
                if dir_to_room.GetLength() < 1e-9:
                    dir_to_room = None
                else:
                    dir_to_room = dir_to_room.Normalize()
            except Exception:
                dir_to_room = None
            
            # Determine if this face is better than current best
            is_better = False
            if best_d is None:
                is_better = True
            elif prefer_facing_away and face_normal is not None and dir_to_room is not None:
                # Prefer face whose normal points AWAY from the room point
                # (i.e., normal · dir_to_room < 0 means normal points away)
                try:
                    dot_cur = float(face_normal.DotProduct(dir_to_room))
                    # Check if current best also has normal info
                    if best_normal is not None:
                        dot_best = float(best_normal.DotProduct(dir_to_room)) if dir_to_room else 0.0
                        # Prefer the face with more negative dot product (pointing away from room)
                        if dot_cur < dot_best - 0.1:
                            is_better = True
                        elif abs(dot_cur - dot_best) < 0.1 and d < best_d:
                            # If both face similar direction, prefer closer
                            is_better = True
                    else:
                        # No normal info for best yet, prefer this if it faces away
                        if dot_cur < 0:
                            is_better = True
                        elif d < best_d:
                            is_better = True
                    ...
                    if d < best_d:
                        is_better = True
            elif (not prefer_facing_away) and face_normal is not None and dir_to_room is not None:
                # Prefer face whose normal points TOWARD the room point
                # (i.e., normal · dir_to_room > 0 means normal points toward room)
                try:
                    dot_cur = float(face_normal.DotProduct(dir_to_room))
                    if best_normal is not None:
                        dot_best = float(best_normal.DotProduct(dir_to_room)) if dir_to_room else 0.0
                        # Prefer the face with more positive dot product (pointing toward room)
                        if dot_cur > dot_best + 0.1:
                            is_better = True
                        elif abs(dot_cur - dot_best) < 0.1 and d < best_d:
                            is_better = True
                    else:
                        # No normal info for best yet, prefer this if it faces toward room
                        if dot_cur > 0:
                            is_better = True
                        elif d < best_d:
                            is_better = True
...
def _place_switch_instance(
    ...
    try:
        ...
        wall_link = None
    if wall_link is not None and isinstance(wall_link, DB.Wall):
        # prefer_facing_away=True ensures we pick the face whose normal points AWAY from the room,
        # so the switch will be embedded IN the wall with its button facing INTO the room.
        link_ref, proj_pt_link = _get_linked_wall_face_ref_and_point(wall_link, link_inst, pt_link, prefer_facing_away=True)
...
def main():
    ...
    # Collect rooms from link - room-based placement (1 room = 1 switch)
    rooms = []
    skipped_corridor_rooms = 0
    for r in link_reader.iter_rooms(link_doc, limit=None, level_id=level_id):
        try:
            # Skip corridor/common areas - they don't need their own switches
            if _is_corridor_room(r, corridor_rx):
                skipped_corridor_rooms += 1
                continue
            # Skip rooms without area (not properly placed)
            if _room_area(r) is None:
                continue
            rooms.append(r)
        except Exception:
            continue
    ...
    if not rooms:
        alert('На выбранном этаже в связи не найдено комнат (кроме коридоров).')
        return
    ...
    output.print_md('Режим: **1 комната = 1 выключатель** (room-based)')
    output.print_md('Комнат на этаже: **{0}** (коридоров пропущено: {1})'.format(len(rooms), skipped_corridor_rooms))
    ...
    # Track already-processed rooms by key (1 room = 1 switch)
    existing_room_keys = set()
    try:
        existing_room_keys = _collect_existing_switch_room_keys(doc, comment_value)
    except Exception:
        existing_room_keys = set()
    ...
    # ROOM-BASED LOOP: iterate over rooms, not doors
    pb_max = len(rooms)
    with forms.ProgressBar(title='ЭОМ: Размещение выключателей по комнатам', cancellable=True, step=1) as pb:
        ...
        for room in rooms:
            ...
            # Check if switch already exists for this room
            room_key = _make_linked_room_key(link_inst, room)
            if room_key and room_key in existing_room_keys:
                skipped_existing += 1
                continue

            # Find all doors for this room
            room_doors = _collect_doors_for_room(room, doors, link_doc)
            if not room_doors:
                skipped_no_doors += 1
                continue

            # Pick the main door (prefer door leading to corridor)
            d, other_room, ph = _pick_main_door_for_room(room, room_doors, corridor_rx, wet_rx)
            if d is None:
                skipped_no_doors += 1
                continue

            # Determine placement: room is the controlled room, decide where to place switch
            # Wet rooms: switch placed OUTSIDE (in corridor/other room)
            # Other rooms: switch placed INSIDE the room
            is_wet = _match_any(wet_rx, _room_name(room))
            if is_wet and other_room is not None:
                place_room = other_room
                controlled_room = room
            else:
                place_room = room
                controlled_room = room

            # For wet rooms, try to find a door that leads to corridor for better placement
            if is_wet:
                for door_cand, other_cand, ph_cand in room_doors:
                    if other_cand is not None and _match_any(corridor_rx, _room_name(other_cand)):
                        d = door_cand
                        place_room = other_cand
                        ph = ph_cand
                        break

            # Extra check: probe rooms if needed
            try:
                ...
                if is_wet:
                    if frp is not None and not _match_any(wet_rx, _room_name(frp)):
                        place_room = frp
                    elif trp is not None and not _match_any(wet_rx, _room_name(trp)):
                        place_room = trp
            except Exception:
                pass

            # Determine switch type based on controlled room
            ...
            # Verify switch point is near the door (max 1500mm XY distance)
            door_center = _door_center_point_link(d)
            if door_center is not None:
                try:
                    dx = float(p_link.X) - float(door_center.X)
                    dy = float(p_link.Y) - float(door_center.Y)
                    dist_xy = (dx * dx + dy * dy) ** 0.5
                    max_dist_ft = mm_to_ft(1500) or 5.0
                    if dist_xy > max_dist_ft:
                        skipped_no_point += 1
                        continue
                except Exception:
                    pass
            ...
            planned.append((symbol, p_host, p_link, d, room_key))

            if len(planned) < max(1, batch_size):
                continue

            with tx('ЭОМ: Разместить выключатели по комнатам', doc=doc, swallow_warnings=True):
                for sym, pt_host, pt_link, door_link, rkey in planned:
                    inst = None
                    try:
                    ...
                    if inst is None:
                        skipped_no_place += 1
                        continue
                    set_comments(inst, _comment_with_room_key(comment_value, rkey))
                    created += 1
                    if rkey:
                        existing_room_keys.add(rkey)
                    try:
    ...
    output.print_md('---')
    output.print_md('Комнат обработано: **{0}**'.format(processed))
    ...
    if skipped_existing:
        output.print_md('Пропущено (уже есть выключатель): **{0}**'.format(skipped_existing))
    if skipped_no_doors:
        output.print_md('Пропущено (нет дверей у комнаты): **{0}**'.format(skipped_no_doors))
    if skipped_no_point:
