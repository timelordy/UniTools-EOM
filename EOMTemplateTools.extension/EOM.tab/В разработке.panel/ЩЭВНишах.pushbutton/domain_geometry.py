...
def align_instance_outer_face_to_boundary(inst, boundary_point_host, inward_normal_host, target_offset_ft=0.0):
    """Move instance so outer-most face lies on boundary plane.

    Plane is defined by `boundary_point_host` and inward XY normal.
    Result target: min_signed_distance_of_bbox_to_plane == target_offset_ft.
    """
...
def _bbox_corners(bbox):
    ...
    for x in xs:
        for y in ys:
            for z in zs:
                pts.append(DB.XYZ(float(x), float(y), float(z)))
    return pts
...
def align_instance_right_face_to_opening(inst,
                                        ...
                                        nudge_mm=0,
                                        clamp_mm=600):
    """Shift instance slightly along tangent so its right edge matches opening right edge.

    Implementation note:
    - We compare *world* projections along `tangent_host` to avoid the bug where
      different local origins break the delta.
    - Clamp is used to keep movement "немного".
    """
    ...
    # Canonicalize tangent direction so that "max" is consistent in world XY.
    # This makes align_mode='max' behave like "right" (mostly +X) on plans.
    try:
        if abs(float(t_host.X)) >= abs(float(t_host.Y)):
            if float(t_host.X) < 0.0:
                t_host = DB.XYZ(-float(t_host.X), -float(t_host.Y), 0.0)
        else:
            if float(t_host.Y) < 0.0:
                t_host = DB.XYZ(-float(t_host.X), -float(t_host.Y), 0.0)
    except Exception:
        pass
    ...
    try:
        ...
        for p_link in _bbox_corners(bb_open):
            try:
                ...
                continue
        open_right = _max_proj_along_dir(open_pts_host, DB.XYZ(0.0, 0.0, 0.0), t_host)
    except Exception:
        open_right = None
    ...
    try:
        ...
        panel_right = _max_proj_along_dir(panel_pts, DB.XYZ(0.0, 0.0, 0.0), t_host)
    except Exception:
        panel_right = None
    if panel_right is None:
        return 0.0, False

    delta = float(open_right) - float(panel_right)
    if abs(delta) < 1e-5:
        return 0.0, True
    ...
    if clamp_ft > 1e-9 and abs(delta) > clamp_ft:
        if debug_output is not None:
            try:
                debug_output.print_md(u'DEBUG: right-face align skipped (delta={:.0f} mm > clamp={} mm)'.format(
                    delta * 304.8,
                    int(clamp_mm or 0)
                ))
            except Exception:
    ...
    try:
        ...
        if debug_output is not None:
            try:
                debug_output.print_md(u'DEBUG: right-face align delta = {:.1f} mm'.format(delta * 304.8))
            except Exception:
                pass
    ...
    for x in xs:
        for y in ys:
            for z in zs:
                d = (float(x) - bx) * nzx + (float(y) - by) * nzy
...
# --- Hotfix overrides (v43): restore stable boundary/edge alignment ---
def align_instance_outer_face_to_boundary(inst, boundary_point_host, inward_normal_host, target_offset_ft=0.0):
    if inst is None or boundary_point_host is None or inward_normal_host is None:
        return 0.0, False
