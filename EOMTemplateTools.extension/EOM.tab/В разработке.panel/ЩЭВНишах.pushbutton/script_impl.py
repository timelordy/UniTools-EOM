...
def main():
    ...
    with tx(u'ЭОМ: ЩЭ в нишах', doc=doc, swallow_warnings=True):
        for level in levels:
            ...
            rooms_on_level = [r for r in rooms if ds.same_level(r, level.Id)]
            apt_count = int(apt_counts.get(level_id_int, 0) or 0)
            info = apt_details.get(level_id_int, {}) or {}
            ...
            # User-friendly apartment summary for the level.
            try:
            ...
            corridor_room, corridor_center = ds.pick_corridor_room(rooms_on_level, corridor_patterns)
            ...
            panel_rule = fpn.select_panel_rule(apt_count, type_rules)
            ...
            # Log which panel type is selected by rules for this level.
            try:
            ...
            opening_point_host = link_transform.OfPoint(opening_point_link)
            corridor_center_host = link_transform.OfPoint(corridor_center)
            tangent_right_host = None

            # Prefer placement by corridor room boundary (finish face), aligned parallel to boundary.
            ...
            if boundary_hit is not None:
                ...
                n_inside_link = dg.pick_interior_normal_for_room(corridor_room, bpt_link, tan_link, boundary_inset_ft)
                # Make normal direction deterministic: always point towards corridor center.
                # This avoids cases where `IsPointInRoom` is unavailable/inconclusive and the
                # normal becomes flipped (then face-align moves the panel *away* from boundary).
                try:
                    to_center = DB.XYZ(
                    ...
                    pass
                # Keep placement on the corridor boundary line (same XY).
                # `boundary_inset_ft` is used only for robust interior-normal picking.
                place_point_link = bpt_link

                place_point_boundary = link_transform.OfPoint(place_point_link)
                boundary_point_host = link_transform.OfPoint(bpt_link)
                inward_normal_host = link_transform.OfVector(n_inside_link) if n_inside_link is not None else None
                tangent_host = link_transform.OfVector(tan_link)
                # Define "right" on plan: choose tangent direction so that "max" corresponds
                # to the visual right side (+X or +Y, whichever dominates).
                ...
                output.print_md(u'•DEBUG: {}: placement=boundary (inset={} мм)'.format(level_name, boundary_inset_mm))
            else:
                place_point_boundary = None
            ...
            if boundary_hit is None:
                host_wall, host_proj, host_dist = dp.find_host_wall_near_point(doc, opening_point_host, wall_search_ft)
                wall_for_offset = host_wall
                wall_for_offset_is_link = False
                if host_proj is not None:
                    place_point = host_proj
                else:
                    link_wall, link_proj_host, _ = dp.find_link_wall_projection(link_doc, link_transform, opening_point_link, wall_search_ft)
                ...
                place_point_outside, outside_shift_ft, outside_ok = dg.outside_point_from_wall(
                    ...
                    wall_offset_ft,
                    link_transform=link_transform if wall_for_offset_is_link else None
                )

            placed_this_level = 0
            while placed_this_level < max_per_level:
                existing = dp.dedupe_existing(doc, symbol, place_point_outside, dedupe_radius_ft)
                if existing is not None:
                    try:
                        loc = existing.Location
                        p0 = loc.Point
                        loc.Point = DB.XYZ(place_point_outside.X, place_point_outside.Y, p0.Z)
                        dp.set_instance_height(existing, host_level or level, panel_height_mm, mm_to_ft)
                        updated_count += 1
                ...
                dp.set_instance_height(inst, host_level or level, panel_height_mm, mm_to_ft)

                # Align instance parallel to corridor boundary / opening.
                try:
                ...
                # Boundary mode: outer face must lie on room boundary, inner side in room.
                try:
                    if force_face_align and boundary_point_host is not None and inward_normal_host is not None:
                ...
                # Boundary mode: align right face of panel to right face of opening (along tangent).
                try:
                    if opening_inst is not None and tangent_host is not None:
                        if right_edge_nudge_mm:
                            output.print_md(u'DEBUG: right-edge nudge = {} mm'.format(int(right_edge_nudge_mm)))
    ...
    _print_result(created_count, updated_count, skipped_dupe, skipped_no_point)
    ...
    report_time_saved(output, 'floor_panel_niches', created_count)
