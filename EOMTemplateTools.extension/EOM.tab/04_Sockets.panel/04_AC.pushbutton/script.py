...
def main():
    ...
    # 1. Select Links
    all_links = link_reader.list_link_instances(doc)
    loaded_links = [l for l in all_links if link_reader.is_link_loaded(l)]
    ...
    picked_names = forms.SelectFromList.show(
        [l.Name for l in loaded_links],
        title="Select Architectural Links",
        multiselect=True
    )
    ...
    link_insts = [l for l in loaded_links if l.Name in picked_names]
