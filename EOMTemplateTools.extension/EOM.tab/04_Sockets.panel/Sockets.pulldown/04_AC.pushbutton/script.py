...
def _is_exterior_wall(wall):
    """Check if wall is exterior/facade by Function parameter or name patterns."""
    if wall is None or (not isinstance(wall, DB.Wall)):
        return False
    # First check Function parameter
    try:
        ...
        pass
    # Fallback: check wall type name for exterior keywords
    exterior_keywords = (
        u'фасад', u'наружн', u'внешн', u'exterior', u'facade', u'external',
        u'curtain', u'витраж', u'ограждающ'
    )
    try:
        wt = wall.WallType
        if wt:
...
def main():
    ...
    try:
        perp_dot_max = float(rules.get('ac_perp_dot_max', 0.5) or 0.5)
    except Exception:
        perp_dot_max = 0.5
    try:
