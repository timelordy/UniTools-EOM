...
from unibim.shortcircuit_globals import get_shortcircuit_globals
...
SCRIPT_PATH = os.path.join(RES_DIR, "ShortCircuit.py")
ICON_PATH = os.path.join(RES_DIR, "TeslaLogo.ico")

if not os.path.exists(SCRIPT_PATH):
    forms.alert(u"Не найден ресурс ShortCircuit.py", title=u"Токи КЗ", warn_icon=True)
    raise SystemExit

doc = revit.doc
uidoc = revit.uidoc

globals_dict = get_shortcircuit_globals(doc=doc, uidoc=uidoc, icon=_load_icon(ICON_PATH))
