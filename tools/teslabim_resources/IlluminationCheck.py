...
class IlluminationCheckWindow(Form):
	...
	def IllCheck_dataGridView1CellContentClick(self, sender, e):
		...
		if self._IllCheck_dataGridView1.CurrentCell.ColumnIndex == 4 and self._IllCheck_dataGridView1.CurrentCell.RowIndex != -1:
			# Вычисляем имя пространства которое нужно показать
			cur_space_name = self._IllCheck_dataGridView1[1, self._IllCheck_dataGridView1.CurrentCell.RowIndex].Value # имя пространства котррое хотим показать
			cur_space_number = self._IllCheck_dataGridView1[0, self._IllCheck_dataGridView1.CurrentCell.RowIndex].Value # номер пространства котррое хотим показать
			# Вытаскиваем Id пространства с таким именем и номером
			for i in spaces_els:
				if GetBuiltinParam(i, BuiltInParameter.ROOM_NAME).AsString() == cur_space_name and GetBuiltinParam(i, BuiltInParameter.ROOM_NUMBER).AsString() == cur_space_number:
					cur_space_id = i.Id
					break
			# Показываем пространство в модели
			ElemsSet = ElementSet() #Создаём пустой набор элементов
			ElemsSet.Insert(doc.GetElement(cur_space_id)) # засовываем в этот набор найденные элементы
			uidoc.ShowElements(ElemsSet)
			# Подсвечиваем элементы в модели
			#Convert list to ICollection
			ele_ids = List[ElementId]([cur_space_id])
			#Select instances (подсвечивает)
	...
	def IlluminationCheckWindowLoad(self, sender, e):
		...
		while a < len(spaces_els_names):
			self._IllCheck_dataGridView1.Rows.Add(spaces_els_numbers[a], spaces_els_names[a], rated_illumination[a], spaces_els_illumination_calculated[a], 'Показать') # Заполняем таблицу исходными данными
			# Красим ячейки
